...
class ActivitySummarizer:
    ...
    async def process(self, packet: ActivityInput) -> ActivitySummary:
        ...
        model_id, _ = self.router.select_model(
            prompt=prompt, require_local=True
        )
        ...
        try:
            ...
            result.needs_sean = parsed.get("needs_sean", result.operator_relevance == Relevance.HIGH)
            result.suggested_surface = parsed.get("suggested_surface", SuggestedSurface.NONE)
            result.confidence = 0.9 if result.category != ActivityCategory.HOUSEKEEPING else 0.7
    ...
    async def process_batch(self, packets: List[ActivityInput]) -> List[ActivitySummary]:
